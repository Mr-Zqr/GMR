...
def load_smplx_file(smplx_file, smplx_body_model_path, downsample_fps=None):
    ...
    def _extract_trackings_dict(npz_data):
        ...
        elif isinstance(trackings, (list, tuple)) and len(trackings) > 0:
            trackings = trackings[0]
    ...
    if "fullpose" in smplx_data and "trans" in smplx_data and "betas" in smplx_data:
        ...
        betas_key = "betas" if "betas" in data_source else "smpl_shapes"
        required_keys = [transl_key, global_orient_key, body_pose_key, betas_key]
        ...
        smplx_betas = np.asarray(data_source[betas_key], dtype=np.float32)
